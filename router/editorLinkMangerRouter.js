...
editorLinkMangerRouter.get("/editorLink", verifyUser, async (req, res) => {
  try {
    ...
    res.status(500).json({
      //   error: "An error occurred while get url list.",
...
editorLinkMangerRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      res.status(500).json({
        // error: "An error occurred while initializing data.",
        message: error.message,
      });
    }
  }
...
//   //   verifyUser,
//   async (req, res) => {
//     //req.body.ids:["6540b52e73dbc9612cada25e","6540b52e73dbc9612cada25f"]
...
//       const hostname = new URL(urlObj.url).hostname; // Extract hostname from URL
//       const alive = await ping.promise.probe(hostname);
//       const editorLink = await EditorLink.findById(urlObj._id);

//       if (editorLink) {
//         editorLink.pingStatus = alive.alive ? "isAlive" : "isDead";
//         editorLink.pingStatusUpdatedAt = new Date();
//         console.log("Alive status: ", alive);
//         await editorLink.save();
//       }

//       return {
//         _id: urlObj._id,
//         url: urlObj.url,
//         pingStatus: editorLink ? editorLink.pingStatus : "Not Found",
//         pingStatusUpdatedAt: editorLink ? editorLink.pingStatusUpdatedAt : null,
//       };
//     });
//     try {
